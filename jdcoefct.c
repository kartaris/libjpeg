...
LJPEG9_METHODDEF(void)
start_output_pass (LJPEG9_j_decompress_ptr cinfo)
{
#ifdef BLOCK_SMOOTHING_SUPPORTED
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
...
decompress_onepass (LJPEG9_j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  ...
  LJPEG9_JSAMPARRAY output_ptr;
  LJPEG9_JDIMENSION start_col, output_col;
...
LJPEG9_METHODDEF(int)
dummy_consume_data (LJPEG9_j_decompress_ptr cinfo)
{
...
LJPEG9_METHODDEF(int)
consume_data (LJPEG9_j_decompress_ptr cinfo)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  LJPEG9_JDIMENSION MCU_col_num;	/* index of current MCU within row */
...
LJPEG9_METHODDEF(int)
decompress_data (LJPEG9_j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  LJPEG9_JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  LJPEG9_JDIMENSION block_num;
  int ci, block_row, block_rows;
  JBLOCKARRAY buffer;
  JBLOCKROW buffer_ptr;
  LJPEG9_JSAMPARRAY output_ptr;
  LJPEG9_JDIMENSION output_col;
  jpeg_component_info *compptr;
  inverse_DCT_method_ptr inverse_DCT;
...
LJPEG9_METHODDEF(int)
decompress_smooth_data (LJPEG9_j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  my_coef_ptr coef = (my_coef_ptr) cinfo->coef;
  LJPEG9_JDIMENSION last_iMCU_row = cinfo->total_iMCU_rows - 1;
  LJPEG9_JDIMENSION block_num, last_block_column;
  ...
  while (cinfo->input_scan_number <= cinfo->output_scan_number &&
    ...
    if (cinfo->input_scan_number == cinfo->output_scan_number) {
       ...
       */
      LJPEG9_JDIMENSION delta = (cinfo->Ss == 0) ? 1 : 0;
