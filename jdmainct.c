...
typedef struct {
  ...
  LJPEG9_JDIMENSION rowgroup_ctr;	/* counts row groups output to postprocessor */
  ...
  LJPEG9_JDIMENSION rowgroups_avail;	/* row groups available to postprocessor */
  LJPEG9_JDIMENSION iMCU_row_ctr;	/* counts iMCU rows to detect image top/bot */
} my_main_controller;
 ...
 */
{
  ...
  int M = cinfo->min_DCT_v_scaled_size;
  jpeg_component_info *compptr;
  LJPEG9_JSAMPARRAY buf, xbuf0, xbuf1;
 ...
 */
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    rows_left = (int) (compptr->downsampled_height % (LJPEG9_JDIMENSION) iMCUheight);
    if (rows_left == 0) rows_left = iMCUheight;
    /* Count nondummy row groups.  Should get same answer for each component,
     * so we need only do it once.
     */
    if (ci == 0) {
      mainp->rowgroups_avail = (LJPEG9_JDIMENSION) ((rows_left-1) / rgroup + 1);
    }
			  ...
			  LJPEG9_JDIMENSION out_rows_avail)
{
  ...
  rowgroups_avail = (LJPEG9_JDIMENSION) cinfo->min_DCT_v_scaled_size;
...
LJPEG9_METHODDEF(void)
process_data_context_main (LJPEG9_j_decompress_ptr cinfo,
			   LJPEG9_JSAMPARRAY output_buf, LJPEG9_JDIMENSION *out_row_ctr,
			   LJPEG9_JDIMENSION out_rows_avail)
{
  my_main_ptr mainp = (my_main_ptr) cinfo->main;
  ...
  case CTX_PREPARE_FOR_IMCU:
    ...
    mainp->rowgroup_ctr = 0;
    mainp->rowgroups_avail = (LJPEG9_JDIMENSION) (cinfo->min_DCT_v_scaled_size - 1);
  ...
  case CTX_PROCESS_IMCU:
    ...
    mainp->rowgroup_ctr = (LJPEG9_JDIMENSION) (cinfo->min_DCT_v_scaled_size + 1);
    mainp->rowgroups_avail = (LJPEG9_JDIMENSION) (cinfo->min_DCT_v_scaled_size + 2);
...
LJPEG9_METHODDEF(void)
process_data_crank_post (LJPEG9_j_decompress_ptr cinfo,
			 LJPEG9_JSAMPARRAY output_buf, LJPEG9_JDIMENSION *out_row_ctr,
			 LJPEG9_JDIMENSION out_rows_avail)
{
  (*cinfo->post->post_process_data) (cinfo, (JSAMPIMAGE) NULL,
				     (LJPEG9_JDIMENSION *) NULL, (LJPEG9_JDIMENSION) 0,
				     output_buf, out_row_ctr, out_rows_avail);
}
