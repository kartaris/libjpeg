...
typedef struct {
  ...
  LJPEG9_JSAMPARRAY buffer;		/* strip buffer, or current strip of virtual */
  LJPEG9_JDIMENSION strip_height;	/* buffer size in rows */
  /* for two-pass mode only: */
  LJPEG9_JDIMENSION starting_row;	/* row # of first row in current strip */
  LJPEG9_JDIMENSION next_row;		/* index of next row to fill/empty in strip */
} my_post_controller;
...
LJPEG9_METHODDEF(void)
post_process_1pass (LJPEG9_j_decompress_ptr cinfo,
		    JSAMPIMAGE input_buf, LJPEG9_JDIMENSION *in_row_group_ctr,
		    LJPEG9_JDIMENSION in_row_groups_avail,
		    LJPEG9_JSAMPARRAY output_buf, LJPEG9_JDIMENSION *out_row_ctr,
		    LJPEG9_JDIMENSION out_rows_avail)
{
  my_post_ptr post = (my_post_ptr) cinfo->post;
  LJPEG9_JDIMENSION num_rows, max_rows;
...
LJPEG9_METHODDEF(void)
post_process_prepass (LJPEG9_j_decompress_ptr cinfo,
		      JSAMPIMAGE input_buf, LJPEG9_JDIMENSION *in_row_group_ctr,
		      LJPEG9_JDIMENSION in_row_groups_avail,
		      LJPEG9_JSAMPARRAY output_buf, LJPEG9_JDIMENSION *out_row_ctr,
		      LJPEG9_JDIMENSION out_rows_avail)
{
  my_post_ptr post = (my_post_ptr) cinfo->post;
  LJPEG9_JDIMENSION old_next_row, num_rows;
  ...
  if (post->next_row > old_next_row) {
    ...
    (*cinfo->cquantize->color_quantize) (cinfo, post->buffer + old_next_row,
					 (LJPEG9_JSAMPARRAY) NULL, (int) num_rows);
    *out_row_ctr += num_rows;
  }
...
LJPEG9_METHODDEF(void)
post_process_2pass (LJPEG9_j_decompress_ptr cinfo,
		    JSAMPIMAGE input_buf, LJPEG9_JDIMENSION *in_row_group_ctr,
		    LJPEG9_JDIMENSION in_row_groups_avail,
		    LJPEG9_JSAMPARRAY output_buf, LJPEG9_JDIMENSION *out_row_ctr,
		    LJPEG9_JDIMENSION out_rows_avail)
...
jinit_d_post_controller (LJPEG9_j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  if (cinfo->quantize_colors) {
     ...
     */
    post->strip_height = (LJPEG9_JDIMENSION) cinfo->max_v_samp_factor;
    if (need_full_buffer) {
