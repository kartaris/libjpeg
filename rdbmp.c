...
typedef struct _bmp_source_struct {
  ...
  LJPEG9_JDIMENSION source_row;	/* Current source row number */
  LJPEG9_JDIMENSION row_width;		/* Physical width of scanlines in file */
...
/* Read the colormap from a BMP file */
{
  ...
  case 4:
    ...
    break;
  default:
    LJPEG9_ERREXIT(sinfo->cinfo, LJPEG9_JERR_BMP_BADCMAP);
    break;
  }
}
...
preload_image (LJPEG9_j_compress_ptr cinfo, LJPEG9_cjpeg_source_ptr sinfo)
{
  ...
  case 32:
    ...
    break;
  default:
    LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_BADDEPTH);
  }
...
#define GET_4B(array,offset)  ((INT32) UCH(array[offset]) + \
  ...
  if (GET_2B(bmpfileheader,0) != 0x4D42) /* 'BM' */
    LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_NOT);
  ...
  if (headerSize < 12 || headerSize > 64)
    LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_BADHEADER);
  ...
  case 64:
    ...
    if (biCompression != 0)
      LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_COMPRESSED);
  ...
  if (biWidth <= 0 || biHeight <= 0)
    LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_EMPTY);
  if (biPlanes != 1)
    LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_BADPLANES);
  ...
  if (mapentrysize > 0) {
    if (biClrUsed <= 0)
      ...
      LJPEG9_ERREXIT(cinfo, LJPEG9_JERR_BMP_BADCMAP);
  ...
  if (source->bits_per_pixel == 24)
    row_width = (LJPEG9_JDIMENSION) (biWidth * 3);
  else if (source->bits_per_pixel == 32)
    row_width = (LJPEG9_JDIMENSION) (biWidth * 4);
  else
    row_width = (LJPEG9_JDIMENSION) biWidth;
  ...
  cinfo->image_width = (LJPEG9_JDIMENSION) biWidth;
  cinfo->image_height = (LJPEG9_JDIMENSION) biHeight;
}
