...
typedef struct {
  ...
  LJPEG9_JDIMENSION rows_to_go;	/* counts rows remaining in source image */
		  ...
		  LJPEG9_JDIMENSION out_row_groups_avail)
{
  ...
  while (*in_row_ctr < in_rows_avail &&
    ...
    numrows = (int) MIN((LJPEG9_JDIMENSION) numrows, inrows);
    (*cinfo->cconvert->color_convert) (cinfo, input_buf + *in_row_ctr,
				       prep->color_buf,
				       (LJPEG9_JDIMENSION) prep->next_buf_row,
				       numrows);
    *in_row_ctr += numrows;
    ...
    if (prep->next_buf_row == cinfo->max_v_samp_factor) {
      (*cinfo->downsample->downsample) (cinfo,
					prep->color_buf, (LJPEG9_JDIMENSION) 0,
		     ...
		     LJPEG9_JDIMENSION out_row_groups_avail)
{
  ...
  while (*out_row_group_ctr < out_row_groups_avail) {
    ...
    if (prep->next_buf_row == prep->next_buf_stop) {
      (*cinfo->downsample->downsample) (cinfo,
					prep->color_buf,
					(LJPEG9_JDIMENSION) prep->this_row_group,
					output_buf, *out_row_group_ctr);
      (*out_row_group_ctr)++;
...
create_context_buffer (j_compress_ptr cinfo)
{
  ...
  int ci, i;
  jpeg_component_info * compptr;
  LJPEG9_JSAMPARRAY true_buffer, fake_buffer;
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
    ...
    true_buffer = (*cinfo->mem->alloc_sarray)
      ((LJPEG9_j_common_ptr) cinfo, JPOOL_IMAGE,
       ...
       (LJPEG9_JDIMENSION) (3 * rgroup_height));
