...
typedef struct {
  ...
  LJPEG9_JSAMPARRAY sv_colormap;	/* colormap allocated at init time */
...
prescan_quantize (LJPEG9_j_decompress_ptr cinfo, LJPEG9_JSAMPARRAY input_buf,
		  LJPEG9_JSAMPARRAY output_buf, int num_rows)
{
  ...
  LJPEG9_JDIMENSION width = cinfo->output_width;
...
/* Master routine for color selection */
{
  ...
  boxlist = (boxptr) (*cinfo->mem->alloc_small)
    ((LJPEG9_j_common_ptr) cinfo, JPOOL_IMAGE, desired_colors * SIZEOF(box));
...
LJPEG9_METHODDEF(void)
pass2_no_dither (LJPEG9_j_decompress_ptr cinfo,
		 LJPEG9_JSAMPARRAY input_buf, LJPEG9_JSAMPARRAY output_buf, int num_rows)
...
LJPEG9_METHODDEF(void)
pass2_fs_dither (LJPEG9_j_decompress_ptr cinfo,
		 LJPEG9_JSAMPARRAY input_buf, LJPEG9_JSAMPARRAY output_buf, int num_rows)
...
/* Allocate and fill in the error_limiter table */
{
  ...
  table = (int *) (*cinfo->mem->alloc_small)
    ((LJPEG9_j_common_ptr) cinfo, JPOOL_IMAGE, (MAXJSAMPLE*2+1) * SIZEOF(int));
...
LJPEG9_METHODDEF(void)
finish_pass1 (LJPEG9_j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
...
LJPEG9_METHODDEF(void)
finish_pass2 (LJPEG9_j_decompress_ptr cinfo)
{
  /* no work */
}
...
LJPEG9_METHODDEF(void)
start_pass_2_quant (LJPEG9_j_decompress_ptr cinfo, boolean is_pre_scan)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
  hist3d histogram = cquantize->histogram;
  int i;
  ...
  if (is_pre_scan) {
	...
	cquantize->fserrors = (FSERRPTR) (*cinfo->mem->alloc_large)
	  ((LJPEG9_j_common_ptr) cinfo, JPOOL_IMAGE, arraysize);
...
LJPEG9_METHODDEF(void)
new_color_map_2_quant (LJPEG9_j_decompress_ptr cinfo)
{
  my_cquantize_ptr cquantize = (my_cquantize_ptr) cinfo->cquantize;
...
LJPEG9_GLOBAL(void)
LJPEG9_jinit_2pass_quantizer (LJPEG9_j_decompress_ptr cinfo)
{
  ...
  cquantize->histogram = (hist3d) (*cinfo->mem->alloc_small)
    ((LJPEG9_j_common_ptr) cinfo, JPOOL_IMAGE, HIST_C0_ELEMS * SIZEOF(hist2d));
  for (i = 0; i < HIST_C0_ELEMS; i++) {
